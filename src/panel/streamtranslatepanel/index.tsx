...
import { pushChunk, startTranscriptSession, streamFinish } from "@/api/streamtranslate";
...
export default function () {
  ...
  const pump = async () => {
    ...
    try {
      ...
      while (audioState.running && buf.length >= chunkSamples) {
        ...
        const j = await pushChunk({ sessionId: audioState.sessionId, float32_16k: chunk })
  ...
  const stopAudioPipeline = async () => {
    ...
    try {
      ...
      if (control.mediaStream) control.mediaStream.getTracks().forEach((t) => t.stop())
    } catch (e) { }
  ...
  const start = async () => {
    ...
    try {
      ...
      audioControl.source = audioControl.audioCtx.createMediaStreamSource(audioControl.mediaStream)

      audioControl.processor = audioControl.audioCtx.createScriptProcessor(4096, 1, 1)
      // const chunkSamples = Math.round(TARGET_SR * (CHUNK_MS / 1000))

      audioControl.processor.onaudioprocess = (e) => {
        if (!audioState.running) return
        const input = e.inputBuffer.getChannelData(0)
        const resampled = resampleLinear(input, audioControl.audioCtx!.sampleRate, TARGET_SR)
  ...
  return (
    <div className={globalStyles.panel}>
      ...
      <div>
        <textarea style={{ width: '100%', height: '200px' }} value={transcriptText}></textarea>
        <textarea style={{ width: '100%', height: '200px' }} value={translationText}></textarea>
      </div>
    </div>
  );
}
